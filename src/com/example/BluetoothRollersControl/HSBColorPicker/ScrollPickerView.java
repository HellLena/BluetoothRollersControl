...
public class ScrollPickerView extends LinearLayout {
	...
	private void init(Context context) {
		barImage = BitmapFactory.decodeResource(context.getResources(),
				com.example.BluetoothRollersControl.R.drawable.com_ttshrk_view_scroll_picker_bar, options);
		this.barSrcRect.set(0, 0, barImage.getWidth(), barImage.getHeight());
		backImage = BitmapFactory.decodeResource(context.getResources(),
				com.example.BluetoothRollersControl.R.drawable.com_ttshrk_view_scroll_picker_background, options);
	...
	public LoopSlotView addSlot(String[] labels, float weight, ScrollType scrollType, String HSBparam) {
		AbstractSlotView tv = SlotViewFactory.create(getContext(), scrollType);
		if(tv instanceof LoopSlotView){
			((LoopSlotView) tv).setHSBparam(HSBparam);
		}
		LayoutParams lp = new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT);
		lp.weight = weight;
		tv.setLabels(labels);
		this.addView(tv, lp);
		return (LoopSlotView)tv;
	}
	...
	public void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		System.out.println("Datetimepicker#onSizeChanged w:" + w + ", h:" + h);
